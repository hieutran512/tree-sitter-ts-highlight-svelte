...
jest.mock("tree-sitter-ts-highlight", () => ({
    highlight: (...args: unknown[]) => highlightMock(...args),
    highlightDiff: (...args: unknown[]) => highlightDiffMock(...args),
}));
...
describe("highlight utilities", () => {
    ...
    test("createHighlightedHtmlStore reacts to code/language changes", () => {
        highlightMock.mockImplementation((code: string, language: string) => `${language}:${code}`);
    ...
    test("createHighlightedDiffHtmlStore reacts with optional options store", () => {
        highlightDiffMock.mockImplementation(
            (oldCode: string, newCode: string, language: string, options?: { view?: string }) =>
                `${language}:${oldCode}->${newCode}:${options?.view ?? "side-by-side"}`,
        );

        const oldCode = writable("a");
        const newCode = writable("b");
        const language = writable("typescript");
        const options = writable<{ view?: "inline" | "side-by-side" } | undefined>({ view: "inline" });
