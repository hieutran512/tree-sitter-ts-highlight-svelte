...
jest.mock("tree-sitter-ts", () => ({
    tokenize: (...args: unknown[]) => tokenizeMock(...args),
}));

jest.mock("tree-sitter-ts-highlight", () => ({
    applyDecorations: (...args: unknown[]) => applyDecorationsMock(...args),
    enhanceSemantics: (...args: unknown[]) => enhanceSemanticsMock(...args),
    groupTokensByLine: (...args: unknown[]) => groupTokensByLineMock(...args),
    createDiffModelWithTokens: (...args: unknown[]) => createDiffModelWithTokensMock(...args),
}));
...
describe("svelte renderer utilities", () => {
    ...
    test("createHighlightRenderable supports semantic highlighting and themes", () => {
        ...
        applyDecorationsMock.mockImplementation((tokens: Array<{ value: string; category: string }>) =>
            tokens.map((token) => ({
                token,
                extraClasses: ["extra"],
